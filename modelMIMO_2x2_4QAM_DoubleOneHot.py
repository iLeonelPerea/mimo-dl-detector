# -*- coding: utf-8 -*-
"""training_2x2_detector_DoubleOneHot.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1wTgvtMoX-oTuJh5Mzft9idac1Q5DeuYC

# MIMO 2x2 Deep Learning Detector Training - One-Hot Per Antenna Strategy

## Description
This notebook implements a Deep Learning-based detector for a 2x2 MIMO communication system using **one-hot encoding per antenna** strategy (also called "double one-hot"). This approach encodes each antenna's symbol separately, providing a balance between accuracy and efficiency.

**Key Feature**: Uses **M×Nt = 8 output neurons**, intermediate between one-hot (16) and label encoding (4).
...
qam_idx = torch.arange(M) + 1
symbol_indices = torch.tensor(
    list(product(qam_idx.numpy(), repeat=Nt)),
    dtype=torch.long
)
...
def generate_training_data(N, symbol_combinations, double_one_hot, SNR_linear, No, Nr, Nt):
...
X_data, y_labels, random_indices = generate_training_data(
    N, symbol_combinations, double_one_hot, SNR_linear, No, Nr, Nt
)
...
def calculate_accuracy(outputs, idx_true, symbol_indices, M):
    ...
    for i in range(len(predicted_indices)):
        matches = (symbol_indices == predicted_indices[i].cpu()).all(dim=1)
...
with torch.no_grad():
    ...
    # Convert to symbol combination indices
    final_predictions = []
    for i in range(len(predicted_indices)):
        matches = (symbol_indices == predicted_indices[i].cpu()).all(dim=1)
        idx_pred = torch.where(matches)[0]
